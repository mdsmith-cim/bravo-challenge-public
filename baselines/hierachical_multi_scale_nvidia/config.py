...
# This is only used if --assets_path is not specified via command line
# Expects to have seg_weights folder, and will create (if training) uniform_centroids folder in here
__C.DEFAULT_ASSETS_PATH = '/default-assets-path'
...
# Expects <DATASET>_DIR folders as listed below to be in this directory if the relevant datasets are used
# This is overridden by --override_dataset_root
__C.DATASET.DATASET_ROOT = '/REPLACE-ME-dataset-root'
#Cityscapes Dir Location
__C.DATASET.CITYSCAPES_DIR = 'Cityscapes-nvidia'
__C.DATASET.CITYSCAPES_CUSTOMCOARSE = 'Cityscapes/autolabelled'
#SDC Augmented Cityscapes Dir Location
__C.DATASET.CITYSCAPES_AUG_DIR = ''
#Mapillary Dataset Dir Location
__C.DATASET.MAPILLARY_DIR = 'mapillary-vistas-1.2'
...
#Bravo Dataset Dir Location
__C.DATASET.BRAVO_DIR = 'bravo_1.0'
# SegmentMeIfYouCan Dir Location
__C.DATASET.SEGMENTMEIFYOUCAN_DIR = 'SegmentMeIfYouCan'
# BDD100K Location
__C.DATASET.BDD100K_DIR = 'bdd100k'
# GTA5 DIR
__C.DATASET.GTA5_DIR = 'GTA5'
# India Driving Dataset dir
__C.DATASET.IDD_DIR = 'IDD_Segmentation'
# WildDash dataset
__C.DATASET.WILDDASH_DIR = 'WildDash2'
# SHIFT Dataset
__C.DATASET.SHIFT_DIR = 'SHIFT'
...
# Use a center crop of size args.pre_size for datasets of varying image sizes during validation e.g. mapillary
# Need to use this if you want to dump images
__C.DATASET.CROP_VAL = False
...
def assert_and_infer_cfg(args, make_immutable=True, train_mode=True):
    ...
    if hasattr(args, 'syncbn') and args.syncbn:
        if args.distributed:
            __C.MODEL.BN = 'SyncBatchNorm'
            __C.MODEL.BNFUNC = torch.nn.SyncBatchNorm
        else:
            raise Exception('No Support for SyncBN without distributed mode')
    else:
    ...
    if args.assets_path is None:
        __C.ASSETS_PATH = __C.DEFAULT_ASSETS_PATH
    else:
    ...
    __C.MODEL.WRN41_CHECKPOINT = os.path.join(WEIGHTS_PATH, 'wider_resnet41_cornflower_sunfish.pth')
    __C.MODEL.X71_CHECKPOINT = os.path.join(WEIGHTS_PATH, 'aligned_xception71.pth')
    __C.MODEL.HRNET_CHECKPOINT = os.path.join(WEIGHTS_PATH, 'hrnetv2_w48_imagenet_pretrained.pth')
    ...
    if args.override_dataset_root is not None:
        __C.DATASET.DATASET_ROOT = args.override_dataset_root
