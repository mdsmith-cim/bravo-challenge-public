...
class RoadAnomaly21(Dataset):
    ...
    def __init__(self, hparams, transforms):
        ...
        self.images = os.listdir(self.img_root)

        if hparams.dataset_mode == 'test':
            
            self.images = [os.path.join(self.img_root, img_path) for img_path in self.images if not ('validation' in img_path)]
            self.labels = [''] * len(self.images)
        elif hparams.dataset_mode == 'val':
            
            self.labels = [os.path.join(self.mask_root, img_path[:-4] + '_labels_semantic.png') for img_path in self.images if 'validation' in img_path]
            self.images = [os.path.join(self.img_root, img_path) for img_path in self.images if 'validation' in img_path]
        elif hparams.dataset_mode == 'all':
            
            self.labels = []
            for img_path in self.images:
                if 'validation' in img_path:
                    self.labels.extend([os.path.join(self.mask_root, img_path[:-4] + '_labels_semantic.png')])
                else:
                    self.labels.extend([''])
                
            self.images = [os.path.join(self.img_root, img_path) for img_path in self.images]
        else:
...
class RoadObstacle21(Dataset):
    ...
    def __init__(self, hparams, transforms):
        ...
        if hparams.dataset_mode == 'test':
            ...
            self.labels = [os.path.join(self.mask_root, img_path[:-5] + '_labels_semantic.png') for img_path in self.images if 'validation' in img_path]
            self.images = [os.path.join(self.img_root , img_path) for img_path in self.images if 'validation' in img_path]
        elif hparams.dataset_mode == 'all':
            ...
            for img_path in self.images:
                if 'validation' in img_path:
                    self.labels.extend([os.path.join(self.mask_root, img_path[:-5] + '_labels_semantic.png')])
                else:
                    self.labels.extend([''])
