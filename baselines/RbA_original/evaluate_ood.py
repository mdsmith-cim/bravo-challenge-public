...
parser.add_argument("--selected_datasets", nargs="*", type=str, default=[],
                    choices=['cityscapes', 'bdd100k', 'road_anomaly', 'fishyscapes_laf', 'fs_static', 'fs_static_v2',
                             'road_anomaly_21', 'road_obstacles', 'lost_and_found'],
                    help="""Names of datasets to be evaluated.
                    """)
parser.add_argument("--score_func", type=str, default="rba", choices=["rba", "pebal", "dense_hybrid", "max_softmax"],
                    help="outlier scoring function to be used in evaluations")
    

args = parser.parse_args()

DATASETS = get_datasets(args.datasets_folder, args.dataset_mode, args.selected_datasets)
...
def get_max_softmax(model, x, **kwargs):
        ...
        return 1 - F.softmax(logits, dim=0).max(dim=0).values
...
def run_evaluations(model, dataset, model_name, dataset_name):
    ...
    elif args.score_func == "max_softmax":
        score_func = get_max_softmax
