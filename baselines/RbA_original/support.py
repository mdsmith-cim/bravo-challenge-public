...
def get_datasets(datasets_folder, dataset_mode, selected_datasets):
    ...
    if dataset_mode == 'all':
        selected_datasets = ['cityscapes', 'bdd100k', 'road_anomaly', 'fishyscapes_laf', 'fs_static', 'fs_static_v2', 'road_anomaly_21', 'road_obstacles', 'lost_and_found']
    ...
    if 'bdd100k' in selected_datasets:
        DATASETS.bdd100k = BDD100KSeg(hparams=bdd100k_config, mode='val', transforms=transform, image_size=(720, 1280))
    if 'road_anomaly' in selected_datasets:
        DATASETS.road_anomaly = RoadAnomaly(hparams=road_anomaly_config, transforms=transform)
    if 'fishyscapes_laf' in selected_datasets:
        DATASETS.fishyscapes_laf = FishyscapesLAF(hparams=fishyscapes_laf_config, transforms=transform)
    if 'fs_static' in selected_datasets:
        DATASETS.fs_static = FishyscapesStatic(hparams=fishyscapes_static_config, transforms=transform, version=1)
    if 'fs_static_v2' in selected_datasets:
        DATASETS.fs_static_v2 = FishyscapesStatic(hparams=fishyscapes_static_config, transforms=transform, version=2)
    ...
    if 'lost_and_found' in selected_datasets:
        DATASETS.lost_and_found = LostAndFound(laf_config, transform)
