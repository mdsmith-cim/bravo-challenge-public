...
parser = argparse.ArgumentParser(description='GTA5 -> Cityscales label converter.')
...
new_mask_dir = gta5_path / 'labels_cityscapes'
new_mask_dir.mkdir(exist_ok=True)

for mask in tqdm(list(mask_dir.glob('*.png')), desc='Converting masks', unit='img'):
    ...
    for label in labels:
        if label.id == -1:
            continue
        new_mask_img_arr[mask_img_arr == label.id] = label.trainId
    new_mask_img = Image.fromarray(new_mask_img_arr, mode='P')
    new_mask_img.putpalette(colormap)
    new_mask_img.save(new_mask_dir / mask.name)